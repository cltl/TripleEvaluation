...
import vu.kafkybot.KafResult;
import vu.kafkybot.TupleElement;
import vu.kafkybot.TupleSaxParser;
...
public class ConvertTuplesToTriples {
    ...
    static public void main (String[] args) {
        ...
        if (!pathToTuplesFile.isEmpty()) {
            if (new File(pathToTuplesFile).isDirectory()) {
                ArrayList<File> files = Util.makeFlatFileList(pathToTuplesFile, extension);
                for (int i = 0; i < files.size(); i++) {
                    File file = files.get(i);
                    convertTupleFileToTriples(file.getAbsolutePath(), firstElement);
                }
            }
            else {
                convertTupleFileToTriples(pathToTuplesFile, firstElement);
            }
        }

    }

    static void convertTupleFileToTriples (String pathToTuplesFile, String firstElement) {
        try {
                        ...
                        if (tupleElement.getName().equals(firstElement)) {
                            for (int k = 0; k < kafResult.getChildren().size(); k++) {
                                if ((k!=j)) {
                                    ...
                                    triple.setTripleId(key+"#"+kafResult.getId());
